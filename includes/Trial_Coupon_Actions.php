...
class Trial_Coupon_Actions {
    ...
    public function create_discount_type( $discount_types ) {
        ...
        $discount_types['subscription_trials'] = __( 'Subscription Trial', 'wcs-trial-coupon' );
    ...
    public function save_coupon_fields( $coupon_id ) {
        ...
        $coupon->add_meta_data( $this->coupon_trial_length, wc_clean( $_POST['_wcs_trial_coupon_length'] ), true );
        $coupon->add_meta_data( $this->coupon_trial_period, wc_clean( $_POST['_wcs_trial_coupon_period'] ), true );
        $coupon->save();
    }
    ...
    public function trial_coupon_validation( $arg1, $coupon, $valid ) {
        ...
        if( $coupon->is_type( 'subscription_trials' ) ){

            $coupon_trial_length = esc_attr( $coupon->get_meta( '_wcs_trial_coupon_length' ) );
            $coupon_trial_period = esc_attr( $coupon->get_meta( '_wcs_trial_coupon_period' ) );
